...
// Usage:
//   /api/debug?symbol=PLTR&source=av
//   /api/debug?symbol=PLTR&source=finnhub
//   /api/debug?symbol=PLTR&source=edgar
//   /api/debug?symbol=PLTR&source=wikipedia
//   /api/debug?symbol=PLTR  (runs all - may timeout on free Vercel tier)

module.exports = async function handler(req, res) {
    res.setHeader('Access-Control-Allow-Origin', '*');
    const symbol = (req.query.symbol || 'CVS').toUpperCase();
    const source = req.query.source || 'all';
    const out = { symbol, source };

    if (source === 'av' || source === 'all') {
        try {
    ...
    if (source === 'finnhub' || source === 'all') {
        try {
            ...
            const m = f.metric || {};
            out.finnhub = {
                name: p.name, employeeTotal: p.employeeTotal,
    ...
    if (source === 'edgar' || source === 'all') {
        try {
            ...
            if (cik) {
                ...
                const niSamples = us_gaap['NetIncomeLoss']
                    ? (us_gaap['NetIncomeLoss'].units.USD || []).filter(e => e.form === '10-K' && e.start && e.end <= today).slice(-3)
                    : null;
                out.edgar = { cik, companyName, today, empSamples, niSamples };
            } else {
    ...
    if (source === 'wikipedia' || source === 'all') {
        try {
            ...
            const searchRes = await fetch(
                'https://en.wikipedia.org/w/api.php?action=opensearch&search=' + encodeURIComponent(cleanName) + '&limit=5&format=json',
            ...
            if (match) {
                const pageRes = await fetch(
                    'https://en.wikipedia.org/w/api.php?action=query&titles=' + encodeURIComponent(match) +
